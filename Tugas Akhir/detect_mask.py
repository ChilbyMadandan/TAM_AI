...
def detect_and_predict_mask(frame, faceNet, maskNet):
    ...
    # buat prediksi ketika wajah muncul
    if len(faces) > 0:
...
while True:
    ...
    for (box, pred) in zip(locs, preds):
        ...
        label = "Mantap pake masker" if mask > withoutMask else "ga pake masker"
        color = (0, 255, 0) if label == "Mantap pake masker" else (0, 0, 255)
